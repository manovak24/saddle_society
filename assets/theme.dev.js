...
(function (scrollLock) {
    ...
    window.theme.readHeights = function () {
      ...
      h.headerInitialHeight = parseInt(document.querySelector('[data-header-height]')?.dataset.height || document.querySelector('[data-header-height]')?.offsetHeight) || 0;
    ...
    function calcVars(checkOrientation = false) {
      ...
      const hasCollectionFilters = document.querySelector('[data-collection-filters]');
      const hasLogoTextLink = document.querySelector('[data-logo-text-link]') !== null;
      ...
      const isCollectionPage = body.classList.contains('template-collection');
      const isSearchPage = body.classList.contains('template-search');
    ...
    function checkFirstSectionTransparency() {
      ...
      return firstSection.classList.contains('supports-transparent-header');
    }

    function isHeaderSticky() {
      return document.querySelector('[data-site-header][data-position="fixed"]');
    }
    ...
    // Delay used in the theme editor to let Shopify's own "scroll to selected block"
    // finish before we run our scroll, otherwise the editor overrides it.
    const EDITOR_SCROLL_DELAY = 400;

    window.theme.scrollTo = (elementTop, element = null) => {
      const getOffsetTop = (top) => {
        const {stickyHeaderHeight} = window.theme.readHeights();
        return Math.round(top + window.scrollY - stickyHeaderHeight);
      };

      // In the theme editor, when a target element is provided, defer the scroll and
      // re-measure from the element so the position is correct after the editor's
      // auto-scroll and any layout shift have settled.
      if (element && window.Shopify?.designMode) {
        setTimeout(() => {
          window.scrollTo({top: getOffsetTop(element.getBoundingClientRect().top), left: 0, behavior: 'auto'});
        }, EDITOR_SCROLL_DELAY);
        return;
      }

      window.scrollTo({top: getOffsetTop(elementTop), left: 0, behavior: 'smooth'});
    };
    ...
    const a11y = {
      ...
      removeTrapFocus: function (options) {
        ...
        document.removeEventListener('focusout', this._onFocusOutHandler);
      ...
      _setupHandlers: function () {
        ...
        if (!this._onFocusOutHandler) {
          this._onFocusOutHandler = this._onFocusOut.bind(this);
        }
    ...
    // Use modern window.scrollY (preferred) with fallback to pageYOffset for older browsers
    ...
    function removeScrollLock() {
      const isPopupVisible = document.body.classList.contains('js-quick-view-visible') || document.body.classList.contains('js-drawer-open-cart');
    ...
    function scrollListener() {
      ...
      // Dispatch initial scroll event after a delay to ensure everything is ready
      setTimeout(() => {
        dispatchScrollEvent();
      }, 100);
    }
    ...
    */
    function loadedImagesEventHook() {
      document.addEventListener(
        'load',
        (e) => {
          if (e.target.tagName.toLowerCase() == 'img' && e.target.classList.contains('is-loading')) {
            e.target.classList.remove('is-loading');
            e.target.parentNode.classList.remove('is-loading');

            const section = e.target.closest('[data-section-type]');
            if (section) section.classList.add('img-in');
          }
        },
        true
      );
    ...
    */
    function removeLoadingClassFromLoadedImages(container) {
      container.querySelectorAll('img.is-loading').forEach((img) => {
        if (img.complete) {
          img.classList.remove('is-loading');
          img.parentNode.classList.remove('is-loading');

          const section = img.closest('[data-section-type]');
          if (section) section.classList.add('img-in');
        }
      });
    }
    ...
    class DeferredLoading extends HTMLElement {
      constructor() {
        ...
        this.container = this;
        if (this.hasAttribute('data-deferred-container')) {
          this.container = this.closest(this.getAttribute('data-deferred-container')) || this.closest('.shopify-section');
        }
      ...
      loadTemplate() {
        ...
        const deferredContent = content.querySelector('[data-deferred-content]');
        if (!deferredContent) return;

        this.append(deferredContent);
        this.setAttribute('data-loaded', true);

        const containsImages = deferredContent.querySelectorAll('img').length > 0;
        if (containsImages) {
      ...
      reloadSrcset(container) {
        ...
        container.querySelectorAll('img').forEach((img) => {
          const reloadSrcsetException = img.parentNode.matches('[data-product-image]');
    ...
    class HoverButtons extends HTMLElement {
      ...
      connectedCallback() {
        if (this.button.classList.contains('btn--text')) return;
    ...
    class SearchForm extends HTMLElement {
      constructor() {
        ...
        this.popdownBody = this.closest('[data-popdown-body]');
        this.popularSearches = this.popdownBody?.querySelector('[data-popular-searches]');
        this.predictiveSearchResults = this.querySelector('[data-predictive-search-results]');
        this.predictiveSearch = this.matches('predictive-search');
        this.searchForm = this.matches('search-form');
        this.selectedElement = null;
        this.activeElement = null;
        this.searchTerm = '';
        this.currentSearchTerm = '';
        this.isSearchPage = document.body.classList.contains('template-search');
        ...
        if (this.isSearchPage) {
          this.mainInputType = document.querySelector('[data-main-input-search] input[name="type"]');
          this.inputType = this.querySelector('input[name="type"]');
      ...
      onChange() {
        this.classList.toggle('is-searched', !this.isFormCleared());
      ...
      onFormSubmit(event) {
        if (!this.getQuery().length || this.querySelector('[aria-selected="true"] a, button[aria-selected="true"]')) event.preventDefault();
      }
      ...
      switchOption(direction) {
        ...
        const predictiveSearchOpened = this.classList.contains('is-searched') && this.predictiveSearchResults;
        ...
        this.selectedElement = visibleElementsContainer.querySelector('[aria-selected="true"]');

        // Filter out hidden elements
        const allVisibleElements = Array.from(visibleElementsContainer.querySelectorAll('[role="option"]')).filter((element) => element.offsetParent !== null);
      ...
      handleFocusableDescendants(reset = false) {
        const selected = this.selectedElement ? this.selectedElement : this.querySelector('[aria-selected="true"]');
    ...
    class PredictiveSearch extends SearchForm {
      ...
      getSearchResults(searchTerm) {
        ...
        fetch(`${theme.routes.predictiveSearchUrl}?q=${encodeURIComponent(searchTerm)}&resources[type]=${resources}&resources[limit]=${suggestionsResultsLimit}&section_id=api-predictive-search`, {
          ...
          .then((text) => {
            const resultsMarkup = new DOMParser().parseFromString(text, 'text/html').querySelector('#shopify-section-api-predictive-search').innerHTML;
      ...
      renderSearchResults(resultsMarkup) {
        ...
        const el = this.predictiveSearchResults.querySelector('[data-search-results-wrapper]');
        const tempId = el.id;

        el.id = `${tempId}--${parentId}`;
        el.setAttribute('data-aos-anchor', `#${el.id}`);
    ...
    */
    function anchorsIntersectionObserver() {
      const anchors = document.querySelectorAll('[data-aos-anchor]');
    ...
    function initAnchorObservers(anchors) {
      ...
      anchors.forEach((anchor) => {
        const rawId = anchor.dataset.aosAnchor;
        if (!rawId) return;
        const elementId = rawId.startsWith('#') ? rawId.slice(1) : rawId;
        const container = document.getElementById(elementId);
        if (container && !anchorObserversCollection.has(container)) {
    ...
    */
    const aosAnchorObserver = new IntersectionObserver(
      (entries, observer) => {
        entries.forEach((entry) => {
          ...
          if (target.hasAttribute('data-aos-intersection')) {
            intersectAt = Number(target.getAttribute('data-aos-intersection'));
          }
          ...
          if (target.hasAttribute('data-aos-debounce')) {
            debounceTime = Number(target.getAttribute('data-aos-debounce'));
          }
    ...
    function onIntersecting(target) {
      ...
      if (target.hasAttribute('data-aos-carousel-desktop') && window.theme.isDesktop()) {
        const slider = target.querySelector('.flickity-slider');
        if (slider) {
          const slide = target.querySelectorAll('[data-slide]');
          const activeSlides = [...slide].filter((slide) => {
            if (slide.matches('.is-selected')) return slide;
          });
          ...
          if (activeSlides.length > 0) {
            ...
            activeSlides[activeSlides.length - 1].setAttribute('data-aos-trigger', `#${triggerId}`);
            inactiveSlides.forEach((slide) => {
              slide.querySelectorAll('[data-aos], [data-aos-anchor]').forEach((element) => {
                element.setAttribute('data-aos-anchor', `#${triggerId}`);
              });
            });
          }
        }
      ...
      if (target.hasAttribute('data-aos-carousel-mobile') && window.theme.isMobile()) {
        const slider = target.querySelector('.carousel--mobile');
        ...
        if (slider) {
          ...
          if (slides.length > 1) {
            slides[0].setAttribute('data-aos-trigger', `#${triggerId}`);
      ...
      if (target.hasAttribute('data-aos-trigger')) {
        const triggerId = target.getAttribute('data-aos-trigger');
        const elementsToTrigger = document.querySelectorAll(`[data-aos-anchor="${triggerId}"]`);
        elementsToTrigger.forEach((element) => animate(element));
      }

      // Animate children
      let elementsToAnimate = target.querySelectorAll('[data-aos]:not(.aos-animate)');
      ...
      if (target.hasAttribute('data-aos-watch-anchors')) {
        ...
        const filteredElements = [...elementsToAnimate].filter((element) => {
          const anchor = element.hasAttribute('data-aos-anchor') ? element.getAttribute('data-aos-anchor') : false;
      ...
      elementsToAnimate.forEach((item) => {
        ...
        let customInit = item.hasAttribute('data-aos-custom-init');
        if (customInit) {
    ...
    function animateTextHighlights(element) {
      const textHighlight = element.querySelectorAll('text-highlight');
      textHighlight.forEach((highlight) => highlight.shouldAnimate());
    }

    function animateCounterUps(element) {
      const counterUps = element.querySelectorAll('text-count-up');
    ...
    */
      ...
      const bodyObserver = new MutationObserver((mutationList) => {
        for (const mutation of mutationList) {
          ...
          const watchMutations =
            target.matches('[id^="api-cart-items-"], [data-collection-filters], [data-collection-products], #AjaxinateLoop, [data-tab="resultsProducts"]') ||
            addedNodes.some((item) =>
              isNode(item) ? item.matches('[id^="api-cart-items-"], [data-collection-filters], [data-collection-products], #AjaxinateLoop, [data-tab="resultsProducts"]') : false
            ) ||
            removedNodes.some((item) =>
              isNode(item) ? item.matches('[id^="api-cart-items-"], [data-collection-filters], [data-collection-products], #AjaxinateLoop, [data-tab="resultsProducts"]') : false
            );
          const isAnimatableElement = target.matches('[data-aos], [data-aos-anchor]') || addedNodes.some((item) => (isNode(item) ? item.matches('[data-aos], [data-aos-anchor]') : false));
          const hasElementToAnimate = target.querySelector(`[data-aos-anchor="#${target.id}"]`) !== null;

          if (isAnimatableElement || hasElementToAnimate || watchMutations) {
            const anchors = target.querySelectorAll('[data-aos-anchor]');
      ...
      if (Shopify.visualPreviewMode) {
            ...
            const mutationObserver = new MutationObserver((mutationList) => {
              for (const mutation of mutationList) {
                if (mutation.type === 'childList') {
                  ...
                  const animatable = mutation.target.querySelectorAll('[data-aos], [data-aos-anchor]');
        ...
        customElements.whenDefined('shopify-visual-preview-section-list').then(() => {
          requestAnimationFrame(() => {
            ...
            if (vpElement) {
              // Force animate all animatable elements immediately for preview
              const animatable = vpElement.querySelectorAll('[data-aos], [data-aos-anchor]');
              animatable?.forEach((element) => animate(element));
            }
          });
        });
    ...
    class TextCountUp extends HTMLElement {
      static observedAttributes = ['data-count-up-init'];

      constructor() {
        super();
        this.container = this;
        this.countUpDuration = this.getAttribute('data-count-up-duration');
      ...
      attributeChangedCallback(name, oldValue, newValue) {
        const countUpInitChange = name === 'data-count-up-init';
      ...
      setTriggerAttribute(active = true) {
        this.setAttribute('data-count-up-init', active);
      }
    ...
    class HoverImages extends HTMLElement {
      ...
      connectedCallback() {
        this.slider = this.querySelector('[data-hover-slider]');
        this.recentlyViewed = this.closest('recently-viewed');
        ...
        if (this.recentlyViewed) {
          if (!this.recentlyViewed.classList.contains('hidden')) {
            this.initBasedOnDevice();
          } else {
            this.onRecentlyViewedLoaded = () => this.initBasedOnDevice();
            this.recentlyViewed.addEventListener('theme:recently-viewed:loaded', this.onRecentlyViewedLoaded);
          }
        } else {
          this.initBasedOnDevice();
        }
      ...
      disconnectedCallback() {
        ...
        if (this.recentlyViewed && this.onRecentlyViewedLoaded) {
          this.recentlyViewed.removeEventListener('theme:recently-viewed:loaded', this.onRecentlyViewedLoaded);
        }
      ...
      addArrowClickHandler() {
        const productLink = this.closest('[data-grid-link]');
        if (productLink) {
          productLink.addEventListener('click', (e) => {
            if (e.target.matches('.flickity-prev-next-button')) {
              e.preventDefault();
            }
          });
        }
      ...
      initTouch() {
        const slidesCount = this.querySelectorAll('[data-hover-slide-touch]').length;
        this.style.setProperty('--slides-count', slidesCount);

        if (this.slider) {
          this.touchController?.abort();
          this.touchController = new AbortController();
          const {signal} = this.touchController;

          this.slider.addEventListener('scroll', this.onScroll, {passive: true, signal});

          // Only intercept touch when there is more than one slide to scroll through,
          // so single-image products don't block ancestor touch gestures (parent
          // carousels, swipe-to-close drawers, etc.).
          if (slidesCount > 1) {
            this.slider.addEventListener('touchstart', (e) => e.stopPropagation(), {passive: true, signal});
          }
      ...
      initFlickity() {
        if (this.flkty || !this.slider || this.slider.classList.contains('flickity-enabled') || this.querySelectorAll('[data-hover-slide]').length < 2) return;

        this.flkty = new window.theme.Flickity(this.slider, {
          cellSelector: '[data-hover-slide]',
          contain: true,
          wrapAround: true,
          watchCSS: true,
          autoPlay: false,
      ...
      videoActions() {
        const youtube = this.querySelector('[data-host="youtube"]');
        const vimeo = this.querySelector('[data-host="vimeo"]');
        const mediaExternal = youtube || vimeo;
        const mediaNative = this.querySelector('video');
      ...
      customElements.define(
        ...
        class HeaderComponent extends HTMLElement {
          ...
          checkWidth() {
            ...
            const wrapperWidth = this.headerWrapper.getBoundingClientRect().width;
            const shouldCompress = wrapperWidth < window.minWidth;
      ...
      customElements.define(
        ...
        class HoverDisclosure extends HTMLElement {
          constructor() {
            ...
            this.expandToggle = this.querySelector('[data-expand-toggle]');
          ...
          connectedCallback() {
            ...
            this.connectExpandToggle();
            this.handleTablets();
            this.connectFirstItemShiftTab();
          }
          ...
          showDisclosure() {
            ...
            this.disclosure.setAttribute('aria-hidden', 'false');
          ...
          connectExpandToggle() {
            if (!this.expandToggle) return;

            this.expandToggle.addEventListener('click', (e) => this.onExpandToggleClick(e));

            document.addEventListener('mousemove', () => { this.expandToggle.blur(); });
          }

          onExpandToggleClick(event) {
            event.preventDefault();

            this.isVisible ? this.hideDisclosure() : this.showDisclosure();

            this.focusFirstItem();
          };

          connectFirstItemShiftTab() {
            if (!this.disclosure || !this.expandToggle) return;

            this.disclosure.addEventListener('keydown', (e) => this.onDisclosureKeydown(e));
          }

          getFirstFocusable() {
            return this.disclosure?.querySelector('a, button, [tabindex]:not([tabindex="-1"])');
          }

          onDisclosureKeydown(event) {
            if (event.key !== 'Tab' || !event.shiftKey) return;
            if (event.target !== this.getFirstFocusable()) return;

            event.preventDefault();

            this.hideDisclosure();
            this.expandToggle.focus();
          };

          focusFirstItem() {
            const target = this.getFirstFocusable();
            if (!target) return;

            const onReady = () => {
              target.focus();
              this.disclosure.removeEventListener('transitionend', onReady);
            };

            this.disclosure.addEventListener('transitionend', onReady, {once: true});
          }
        }
      );
    }
    ...
    */
      ...
      customElements.define(
        ...
        class CartItems extends HTMLElement {
          ...
          connectedCallback() {
            ...
            this.cartDrawerEnabled = settings$5.cartDrawerEnabled;
          ...
          assignArguments() {
            ...
            this.cartWidgetsHolder = this.cart.querySelectorAll('[data-cart-widget-holder]');
          ...
          checkForMaxInventoryError() {
            let errorVariantTitle = sessionStorage.getItem('cart:maxInventoryError');

            if (errorVariantTitle) {
              sessionStorage.removeItem('cart:maxInventoryError');
            } else if (window.theme?.outOfStockProductTitle) {
              errorVariantTitle = window.theme.outOfStockProductTitle;
            }
          ...
          updateCart(updateData = {}, currentItem = null) {
            ...
            // Shopify line indexes are 1-based; 0 means the line item could not be resolved
            if (!lineIndex) return;
          ...
          resetLineItem(item) {
            ...
            item.classList.remove('is-loading', 'is-removed');
          }
          ...
          toggleClassesOnContainers() {
            ...
            if (this.isCartPage) {
              ...
              this.cartWidgetsHolder.forEach((holder) => {
                holder.classList.toggle('hidden', !that.hasItemsInCart());
              });
            }
          ...
          formSubmitHandler(event) {
            ...
            const termsAccepted = this.cartTermsCheckbox.checked;
            const termsError = this.cart.querySelector('[data-terms-error-message]');

            if (!termsAccepted) {
              if (termsError?.classList.contains('is-expanded')) {
                event?.preventDefault();
                return;
              }
      ...
      customElements.define(
        ...
        class DrawerElement extends HTMLElement {
          ...
          disconnectedCallback() {
            ...
            // If drawer was open when disconnected (e.g., section refresh in theme editor), clean up
            if (this.isOpen()) {
              this.body.classList.remove('js-drawer-open', 'js-drawer-open--inner', 'js-drawer-open--inner-qv');
              this.dispatchOnDocument('theme:scroll:unlock');
              this.dispatchOnDocument('theme:drawer:close');
            }
          ...
          dispatchOnDocument(name, detail) {
            document.dispatchEvent(new CustomEvent(name, {bubbles: true, ...(detail !== undefined ? {detail} : {})}));
          }

          isRelevantTransition(event, propertyNames) {
            if (event.target !== this) return false;
            if (!propertyNames) return true;
            return propertyNames.includes(event.propertyName);
          }

          startQuickviewTransition() {
            this._isTransitioning = true;
            this._transitionTimeout = setTimeout(() => this.clearQuickviewTransition(), 500);
          }

          isInnerDrawer(triggerControls) {
            return Boolean(
              triggerControls?.includes('size-chart') ||
              triggerControls?.includes('popup-text') ||
              triggerControls?.includes('ProductAccordion')
            );
          }
          ...
          handleDrawerOpening(e) {
            ...
            if (!trigger || !trigger.hasAttribute('data-mobile-menu-trigger')) return;
          ...
          isQuickviewInnerDrawer() {
            return this.closest('[data-quick-view-item]') &&
              (this.classList.contains('popup-chart') || this.classList.contains('product-popup') || this.id?.includes('ProductAccordion'));
          }
          ...
          handleToggle(e) {
            ...
            // Block rapid clicks on quickview inner drawers during transitions
            if (this.isQuickviewInnerDrawer() && this._isTransitioning) return;
          ...
          handleOutsideClick(e) {
            ...
            const allOpenDrawers = Array.from(document.querySelectorAll('drawer-element.is-open'));
            if (allOpenDrawers.length === 0) return;

            const innermostDrawer = allOpenDrawers.find((drawer) => {
              return !allOpenDrawers.some((other) => other !== drawer && drawer.contains(other));
            });

            if (innermostDrawer !== this) return;
          ...
          open(e) {
            const trigger = e?.detail?.element || e?.target;
            const drawerScroller = this.querySelector('[data-scroll]') || this;
            const triggerControls = trigger?.getAttribute('aria-controls');
            const isInnerDrawer = this.isInnerDrawer(triggerControls);
            const isQvInner = this.isQuickviewInnerDrawer();

            // For quickview inner drawers: clear any pending transition and close other open inner drawers
            if (isQvInner) {
              this.clearQuickviewTransition();

              const container = this.closest('[data-quick-view-item]');
              container?.querySelectorAll('drawer-element.is-open').forEach((drawer) => {
                if (drawer !== this && drawer.isQuickviewInnerDrawer()) {
                  drawer.closeQuickviewImmediately();
                }
              });
            }
            ...
            this.dispatchOnDocument('theme:scroll:lock', drawerScroller);
            this.dispatchOnDocument('theme:drawer:open');
            ...
            const onOpenTransitionEnd = (event) => {
              if (!this.isRelevantTransition(event, ['transform', 'top'])) return;

              if (this.isQuickviewInnerDrawer()) {
                this.clearQuickviewTransition();
              }

              this.a11y.state.trigger = trigger;
              this.a11y.trapFocus({container: this});
              this.removeEventListener('transitionend', onOpenTransitionEnd);
            };

            if (isQvInner) {
              this.startQuickviewTransition();
            }

            this.addEventListener('transitionend', onOpenTransitionEnd);
          }

          clearQuickviewTransition() {
            clearTimeout(this._transitionTimeout);
            this._isTransitioning = false;
          }

          closeQuickviewImmediately() {
            if (!this.isOpen()) return;
          ...
          close(event = false) {
            ...
            const allOpenDrawers = document.querySelectorAll('drawer-element.is-open');
            const isLastOpenDrawer = allOpenDrawers.length === 1;

            document.querySelectorAll('drawer-toggle').forEach((toggle) => toggle.updateAriaExpanded(false));

            this.a11y.removeTrapFocus({container: this});
            this.classList.remove('is-open');

            const onCloseTransitionEnd = (e) => {
              if (!this.isRelevantTransition(e)) return;
            ...
            if (isLastOpenDrawer) {
              this.body.classList.remove('js-drawer-open', 'js-drawer-open--inner');
            }
          ...
          initMobileMenuBehavior() {
            this.initMobileSearchBehavior();

            if (theme.settings.mobileMenuBehaviour === 'link') return;

            this.initMobileNavLinkBehavior();
          }

          initMobileSearchBehavior() {
            ...
            navSearchOpen?.forEach((element) => {
              element.addEventListener('click', (event) => {
                ...
                const popdownToggle = this.resolvePopdownToggle();

                document.addEventListener('theme:drawer:close', () => popdownToggle?.click(), {once: true});
          ...
          resolvePopdownToggle() {
            ...
            const isHeaderCompressed = document.querySelector('[data-site-header]')?.classList.contains('site-header--compress');
            const mobileMenuToggle = '[data-mobile-menu] [data-popdown-toggle]';
            const compressToggle = '[data-nav-items-compress] [data-popdown-toggle]';
            const iconsToggle = '[data-nav-icons] [data-popdown-toggle]';

            const toggleSelectors = isTouch
              ? [mobileMenuToggle]
              : isHeaderCompressed
                ? [compressToggle, iconsToggle, mobileMenuToggle]
                : [iconsToggle, compressToggle, mobileMenuToggle];

            return toggleSelectors.reduce((found, selector) => found || document.querySelector(selector), null);
          }

          initMobileNavLinkBehavior() {
            const navMobileLinks = this.querySelectorAll('[data-nav-link-mobile]');
            if (!navMobileLinks.length) return;
        ...
        class DrawerToggle extends HTMLElement {
          ...
          handleClick(e) {
            ...
            if (this.button.hasAttribute('data-mobile-menu-trigger')) {
              this.handleMobileMenuTrigger();
            } else {
              this.handleStandardToggle();
            }
          }

          handleMobileMenuTrigger() {
            document.dispatchEvent(
              new CustomEvent('theme:drawer:opening', {
                bubbles: true,
    ...
    class DeferredMedia extends HTMLElement {
      constructor() {
        ...
        this.autoplayVideo = this.getAttribute('data-autoplay-video') === 'true';
        this.mediaContainer = this.closest('[data-media-id]');
      ...
      connectedCallback() {
        const deferredMediaButton = this.querySelector('[data-deferred-media-button]');
      ...
      loadContent() {
        ...
        this.mediaContainer.classList.add('is-loading');
        const content = document.createElement('div');
        ...
        this.element.addEventListener('load', () => {
          this.mediaContainer.classList.remove('is-loading');
        });
        ...
        let observer = new IntersectionObserver(
          (entries, observer) => {
            entries.forEach((entry) => {
              ...
              const isVisible = !this.element.closest('.media--hidden');
      ...
      pauseOtherMedia() {
        const currentMedia = `[data-media-id="${this.mediaId}"]`;
        const mediaGroup = this.closest('[data-product-single-media-group]');
        const otherMedia = mediaGroup.querySelectorAll(`[data-product-single-media-wrapper]:not(${currentMedia})`);

        if (otherMedia.length) {
          otherMedia.forEach((media) => {
            // Dispatch event to pause videos/models in other media
      ...
      customElements.define(
        ...
        class CollapsibleElements extends HTMLElement {
          constructor() {
            ...
            this.single = this.hasAttribute('data-collapsible-single');
            this.group = this.getAttribute('data-collapsible-group');
            this.triggers = this.querySelectorAll('[data-collapsible-trigger]');
            this.resetHeightTimer = 0;
            this.isTransitioning = false;
            this.transitionOverride = this.closest('[data-collapsible-transition-override]');
          ...
          handleEllipsisToggle(event) {
            ...
            const productAccordion = event.target.closest('[data-collapsible]');

            if (!productAccordion) return;

            const trigger = productAccordion.querySelector('[data-collapsible-trigger]');
            if (!trigger) return;
            ...
            const heightElement = productAccordion.querySelector('[data-height]');
          ...
          collapsibleToggle(e) {
            ...
            const trigger = e.target.matches('[data-collapsible-trigger]') ? e.target : e.target.closest('[data-collapsible-trigger]');
            const dropdownId = trigger.getAttribute('aria-controls');
            const dropdown = document.getElementById(dropdownId);
            const triggerMobile = trigger.hasAttribute('data-collapsible-trigger-mobile');
          ...
          closeOtherItems(trigger) {
            const triggerContainer = trigger.closest('[data-collapsible-single]');

            // First, close items within the same collapsible-elements component
            this.triggers.forEach((otherTrigger) => {
              // Skip if it's the same trigger or not expanded
              if (trigger === otherTrigger || !otherTrigger.classList.contains('is-expanded')) {
                return;
              }

              // Skip if triggers are in different parent containers
              const otherTriggerContainer = otherTrigger.closest('[data-collapsible-single]');
              if (triggerContainer !== otherTriggerContainer) {
                return;
              }

              // Close the expanded item
              const dropdown = document.getElementById(otherTrigger.getAttribute('aria-controls'));
              requestAnimationFrame(() => this.closeItem(dropdown, otherTrigger));
            });

            // If this component has a group attribute, also close items in other collapsible-elements with the same group
            if (this.group) {
              const allCollapsibleElements = document.querySelectorAll('collapsible-elements');
              allCollapsibleElements.forEach((otherCollapsible) => {
                // Skip self
                if (otherCollapsible === this) return;

                // Only process if it has the same group
                const otherGroup = otherCollapsible.getAttribute('data-collapsible-group');
                if (otherGroup !== this.group) return;

                // Close all expanded triggers in the other component
                const otherTriggers = otherCollapsible.querySelectorAll('[data-collapsible-trigger]');
                otherTriggers.forEach((otherTrigger) => {
                  if (!otherTrigger.classList.contains('is-expanded')) return;

                  const dropdown = document.getElementById(otherTrigger.getAttribute('aria-controls'));
                  if (dropdown) {
                    requestAnimationFrame(() => {
                      otherCollapsible.closeItem(dropdown, otherTrigger);
                    });
                  }
                });
              });
          ...
          openItem(dropdown, trigger) {
            const dropdownHeight = dropdown.querySelector('[data-collapsible-content]').offsetHeight;
          ...
          handleTransitioning(dropdown, isExpanded = false) {
            ...
            if (isExpanded) {
              this.resetHeightTimer = setTimeout(() => {
                const toggleEllipsis = dropdown.querySelector('toggle-ellipsis');
          ...
          onResize() {
            if (!this.querySelector('.is-expanded')) return;

            const expandedDropdowns = Array.from(document.querySelectorAll('[data-collapsible]:has(.is-expanded)'));
            const openedEllipsis = document.querySelectorAll('[data-collapsible]:has(toggle-ellipsis.is-open)');

            expandedDropdowns.forEach((dropdown) => {
              const toggleEllipsis = dropdown.querySelector('toggle-ellipsis');
              if (toggleEllipsis?.classList.contains('is-open')) {
          ...
          updateDropdownHeight = (dropdown, ellipsis = false) => {
            const container = dropdown.querySelector('[data-collapsible-container]');
            const toggleEllipsis = dropdown.querySelector('toggle-ellipsis');
            const actions = toggleEllipsis.querySelector('[data-actions]');
      ...
      customElements.define(
        ...
        class CustomScrollbar extends HTMLElement {
          constructor() {
            ...
            this.scrollbarItems = this.querySelector('[data-custom-scrollbar-items]');
            this.scrollbar = this.querySelector('[data-custom-scrollbar]');
            this.scrollbarThumb = this.querySelector('[data-custom-scrollbar-thumb]');
            this.trackWidth = 0;
          ...
          connectedCallback() {
            ...
            if (this.scrollbarItems.children.length) {
              this.calculateTrack(this.scrollbarItems.querySelector('.current'));
            }
          }
    ...
    class SearchPopdown extends HTMLElement {
      constructor() {
        super();
        this.isPopdownInHeader = this.hasAttribute('data-popdown-in-header');
        this.isPopdownInPage = this.hasAttribute('data-popdown-in-page');
        this.popdownBody = this.querySelector('[data-popdown-body]');
        this.popdownClose = this.querySelector('[data-popdown-close]');
        this.searchFormInner = this.querySelector('[data-search-form-inner]');
        this.popularSearchesLink = this.querySelectorAll('[data-popular-searches-link]');
        this.searchFormWrapper = this.querySelector('search-form') ? this.querySelector('search-form') : this.querySelector('predictive-search');
        this.predictiveSearch = this.searchFormWrapper.matches('predictive-search');
        this.header = document.querySelector('[data-site-header]');
        this.headerSection = this.header?.parentNode;
        this.nav = this.header?.querySelector('[data-nav]');
        this.mobileMenu = this.headerSection?.querySelector('[data-mobile-menu]');
        this.a11y = window.theme.a11y;
        ...
        if (this.isPopdownInHeader) {
          ...
          this.popdownToggle = this.querySelector('[data-popdown-toggle]');
        }
      }
      ...
      popdownTransitionCallback(event) {
        ...
        if (!this.details.classList.contains('is-open')) {
          this.onClose();
      ...
      onBodyClick(event) {
        ...
        const isHeaderMenuOpened = this.header?.classList.contains('site-header--menu-opened');
      ...
      triggerPopdownOpen(event) {
        ...
        if (shouldOpenPopdownOnTouchDevice) {
          ...
          const isHeaderCompressed = this.header.classList.contains('site-header--compress');
          let popdownToggle = this.mobileMenu.querySelector('[data-popdown-toggle]');

          if (!isTouch) {
            popdownToggle = isHeaderCompressed ? this.nav.querySelector('[data-nav-items-compress] [data-popdown-toggle]') : this.nav.querySelector('[data-nav-icons] [data-popdown-toggle]');
          }
      ...
      close(event) {
        ...
        const ariaActivedescendant = this.searchFormWrapper.input.getAttribute('aria-activedescendant');
    ...
    class ToggleEllipsis extends HTMLElement {
      constructor() {
        ...
        this.initialHeight = this.getAttribute('data-height');
        this.content = this.querySelector('[data-content]');
        this.actions = this.querySelector('[data-actions]');
      ...
      toggleActions() {
        this.classList.toggle('is-enabled', this.content.offsetHeight + this.actions.offsetHeight > this.initialHeight);
        this.updateTabIndex();
      }

      toggleActionsEvent() {
        const isCollapsibleExpanded = this.closest('[data-collapsible-container]').getAttribute('aria-expanded') === 'true';
        ...
        class NativeScrollbar extends HTMLElement {
          ...
          connectedCallback() {
            ...
            if (this.collectionSwatchStyle === 'text-slider') {
              ...
              this.addEventListener('mouseleave', () => {
                this.isHovering = false;
                this.arrowPrev?.classList.add('is-hidden');
                this.arrowNext?.classList.add('is-hidden');
              });
            }
          }
          ...
          togglePrevArrow() {
            ...
            requestAnimationFrame(() => {
              this.arrowPrev.classList.toggle('is-hidden', Math.floor(this.scrollbar.scrollLeft) <= 0);
            });
          }
    ...
    if (!customElements.get('popout-select')) {
      customElements.define(
        ...
        class Popout extends HTMLElement {
          ...
          connectedCallback() {
            this.popout = this;
            this.selectorWrapper = this.popout.closest('[data-selector-wrapper]');
            this.popoutList = this.popout.querySelector('[data-popout-list]');
            this.popoutToggle = this.popout.querySelector('[data-popout-toggle]');
            this.popoutText = this.popout.querySelector('[data-popout-text]');
            this.popoutInput = this.popout.querySelector('[data-popout-input]');
            this.popoutOptions = this.popout.querySelectorAll('[data-popout-option]');
            this.popoutPrevent = this.popout.getAttribute('data-popout-prevent') === 'true';
            this.cartItem = this.popoutList.closest('[data-cart-item]');
          ...
          popupToggleClick(evt) {
            ...
            if (this.popoutList.closest('[data-product-grid-item]')) {
              const productGridItemImage = this.popoutList.closest('[data-product-grid-item]').querySelector('[data-product-image]');
            ...
            if (this.cartItem) {
              this.cartItem.classList.toggle('cart__item--open');
            }

            evt.currentTarget.setAttribute('aria-expanded', !ariaExpanded);
            this.popoutList.classList.toggle('select-popout__list--visible');
            this.toggleListPosition();
            this.popupListSetDimensions();
          ...
          popupToggleFocusout(evt) {
            const ariaExpanded = this.popoutToggle.getAttribute('aria-expanded') === 'true';
            ...
            const popoutFromQuickView = evt.relatedTarget.hasAttribute('data-quick-view-inner') || evt.relatedTarget.hasAttribute('data-quick-view-focus');
          ...
          popupListFocusout(evt) {
            ...
            const isVisible = this.popoutList.classList.contains('select-popout__list--visible');
          ...
          calc(get = 'shouldBeOnTop') {
            ...
            const quickViewItem = this.popout.closest('[data-quick-view-item]');
            const cartDrawer = this.popout.closest('[data-cart-drawer]');
            ...
            if (quickViewItem) {
              const quickViewFooter = quickViewItem.querySelector('[data-quick-view-foot]');
            ...
            if (cartDrawer) {
              const cartDrawerMessage = cartDrawer.querySelector('[data-cart-message]');
              const cartDrawerErrors = cartDrawer.querySelector('[data-cart-errors]');
              const cartDrawerBody = cartDrawer.querySelector('[data-cart-drawer-body]');
              const cartDrawerItems = cartDrawer.querySelector('[data-items-holder]');
          ...
          toggleListPosition() {
            ...
            if (ariaExpanded) {
              this.selectorWrapper?.classList.add('selector-wrapper--open');
            } else {
              this.selectorWrapper?.classList.remove('selector-wrapper--open');
            }

            if (shouldBeOnTop) {
              this.popoutList.classList.add('select-popout__list--top');
              this.popoutTop = true;
            } else {
              this.popoutList.classList.remove('select-popout__list--top');
              this.popoutTop = false;
            }
          }
          ...
          popupOptionsClick(evt) {
            ...
            if (link.attributes.href.value === '#') {
              ...
              if (!this.popoutInput && this.popout.nextSibling.hasAttribute('data-quantity-holder')) {
                this.popoutInput = this.popout.nextSibling.querySelector('[data-quantity-field]');
              }
              ...
              if (this.popoutPrevent) {
                ...
                changeEvent.fromCartBar = evt.detail.fromCartBar;
                this.popoutInput.dispatchEvent(changeEvent);
                ...
                const currentElement = this.popoutList.querySelector('[class*="--current"]');
                let targetClass = '--current';

                if (currentElement && currentElement.classList.length) {
                  for (const currentElementClass of currentElement.classList) {
                    if (currentElementClass.includes('--current')) {
          ...
          switchQuantityInputBehavior(prevent = true) {
            if (prevent || !this.popoutInput.hasAttribute('data-quantity-field')) return;

            const targetElement = this.popoutList.querySelector(`[data-value="${this.popoutInput.value}"]`);
            if (!targetElement) return;
            if (targetElement.parentElement.nextSibling) return;

            this.popout.classList.add('is-active');
            this.selectorWrapper?.classList.remove('selector-wrapper--open');
          }
          ...
          _connectToggle() {
            this.popout.setAttribute('data-popout-initialized', '');
          ...
          _submitForm() {
            const form = this.popout.closest('form');
            if (form) {
              form.submit();
            }
          }
          ...
          _hideList() {
            this.popoutList.classList.remove('select-popout__list--visible');
            this.popoutToggle.setAttribute('aria-expanded', false);
            this.toggleListPosition();

            if (this.cartItem) {
              this.cartItem.classList.remove('cart__item--open');
            }
          }
      ...
      customElements.define(
        ...
        class QuantityCounter extends HTMLElement {
          ...
          connectedCallback() {
            // DOM Elements
            this.quantity = this;
            this.cartItem = this.quantity.closest('[data-cart-item]');

            if (!this.quantity) {
              return;
            }

            this.field = this.quantity.querySelector('[data-quantity-field]');
            this.buttons = this.quantity.querySelectorAll('[data-quantity-button]');
            this.increaseButton = this.quantity.querySelector('[data-quantity-plus]');
            ...
            // Events
            if (!this.quantity.classList.contains('read-only')) {
          ...
          changeValueOnClick() {
            this.buttons.forEach((element) => {
              element.addEventListener('click', (event) => {
                ...
                const clickedElement = event.target;
                const isDescrease = clickedElement.matches('[data-quantity-minus]') || clickedElement.closest('[data-quantity-minus]');
                const isIncrease = clickedElement.matches('[data-quantity-plus]') || clickedElement.closest('[data-quantity-plus]');
          ...
          disableIncrease() {
            this.increaseButton.classList.toggle('is-disabled', this.quantityValue >= this.maxValue && this.maxValue !== null);
          }
    ...
    if (!customElements.get('tabs-component')) {
      customElements.define(
        ...
        class GlobalTabs extends HTMLElement {
           ...
           */
          handleTabsHistory() {
            this.onHistoryChangeBound = this.onHistoryChangeBound || this.onHistoryChange.bind(this);
    ...
    if (!customElements.get('text-highlight')) {
      customElements.define(
        ...
        class TextHighlight extends HTMLElement {
          static observedAttributes = ['data-is-active'];
          ...
          constructor() {
            ...
            this.link = this.closest('[data-has-highlight]');
            this.type = this.getAttribute('data-highlight-type');
            this.highlightHolder = this.querySelector('[data-highlight-holder]');
            this.textCountUp = this.closest('text-count-up');
            this.sup = this.querySelector('sup');
            this.textRevealCropper = this.closest('.text-reveal__cropper');

            this.target = this.highlightHolder;
            if (this.type === 'circle-hand-drawn' || this.type === 'circle' || this.type === 'underline-hand-drawn') {
              this.target = this.querySelector('path');
            }
           ...
           */
          attributeChangedCallback(name, oldValue, newValue) {
            const isActiveChange = name === 'data-is-active';
          ...
          triggerAnimation() {
            const parentAnimation = this.closest('[data-aos]');
            if (!this.animation) {
              requestAnimationFrame(() => {
            ...
            if (parentAnimation) {
              if (parentAnimation.classList.contains('aos-animate')) {
                this.textRevealCropper?.classList.remove('overflow-hidden');
                this.setAnimationDelay(0);
                this.animate();
                return;
              }

              this.textRevealCropper?.classList.add('overflow-hidden');
          ...
          shouldAnimate() {
            ...
            requestAnimationFrame(() => {
              ...
              const inSlider = Boolean(this.closest('.flickity-enabled'));
              if (inSlider) shouldAnimate = Boolean(this.closest('[data-slide].is-selected'));

              const stickyTextItem = this.closest('[data-sticky-text]');
              if (stickyTextItem) shouldAnimate = Boolean(stickyTextItem.classList.contains('is-active'));
          ...
          setTriggerAttribute(active = true) {
            this.setAttribute('data-is-active', active);
          }
          ...
          onFinish() {
            ...
            if (this.hasAttribute('data-is-active')) {
              const delay = this.getAttribute('data-is-active') === 'true' ? 0 : settings$2[this.type].timing.delay;
    ...
    class ProductModel extends window.theme.DeferredMedia {
      ...
      loadContent() {
        ...
        this.xrButton = this.mediaContainer.closest('[data-product-single-media-group]')?.parentElement.querySelector('[data-shopify-xr]');
      ...
      setupModelViewerListeners() {
        this.mediaContainer.addEventListener('theme:media:visible', () => {
          this.xrButton.setAttribute('data-shopify-model3d-id', this.modelId);
    ...
    const loaderHTML = `<div class="pswp__loader"><div class="loader loader--image"><div class="loader__image"></div></div></div>`;

    class LoadPhotoswipe {
      constructor(items, options = '', templateIndex = 0, triggerButton = null) {
        this.items = items;
        this.triggerBtn = triggerButton;
        this.pswpElements = document.querySelectorAll('.pswp');
      ...
      loadingState(event) {
        if (event === this.options.index && this.pswpElement.classList.contains('pswp--custom-loader')) {
      ...
      initPopupCallback() {
        ...
        if (this.pswpElement.classList.contains('pswp--notification')) {
          new LoadNotification(this.popup, this.pswpElement);
        }

        this.closePopup = () => {
          if (this.pswpElement.classList.contains('popup-quick-view')) {
            this.pswpElement.classList.add('popup-quick-view--animate-out'); // Close the Quickview popup accordingly
          } else {
            this.popup.close();
          }
        };
      ...
      initVideo() {
        const videoContainer = this.pswpElement.querySelector('.pswp__custom-iframe');
        if (videoContainer) {
        ...
        if (this.popupThumbsContainer && this.popupThumbsContainer.firstChild) {
          ...
          this.popupThumbs.forEach((element, i) => {
            element.addEventListener('click', (e) => {
              e.preventDefault();
              element.parentElement.querySelector('.is-current').classList.remove('is-current');
      ...
      hideUnusedButtons() {
        const buttons = this.pswpElement.querySelectorAll('.pswp__button, .pswp__caption-close');
      ...
      onClose() {
        ...
        if (!document.body.classList.contains('js-drawer-open-cart')) {
          this.a11y.removeTrapFocus();
        }
        ...
        setTimeout(() => {
          ...
          const isCartDrawerOpen = document.body.classList.contains('js-drawer-open-cart');
      ...
      recentlyOpenedPopupsCount() {
        ...
        this.pswpElements.forEach((popup) => {
          const isOpened = popup.classList.contains('pswp--open');
    ...
    class Accessibility {
      ...
      focusEvents() {
        document.addEventListener('keyup', (event) => {
          ...
          this.body.classList.remove('no-outline');
          this.isFocused = true;
        });
      }
      ...
      focusEventsOff() {
        document.addEventListener('mousedown', () => {
          this.body.classList.add('no-outline');
          this.isFocused = false;
        });
      }
    ...
    class GridSwatch extends HTMLElement {
      ...
      connectedCallback() {
        this.outer = this.closest('[data-product-block]');
        this.swatchElements = this.querySelectorAll('[data-swatch]');

        this.swatchElements.forEach((el) => {
          const image = el.getAttribute('data-swatch-image');
          const swatchButton = el.closest('[data-swatch-button]');
          const swatchLink = swatchButton?.querySelector('a[data-swatch-variant]');

          if (swatchLink) {
            this.handleClicks(swatchLink);

            if (!image) {
              swatchLink.classList.add('swatch__link--no-image');
            }
          }
        });
      ...
      handleShowMore() {
        ...
        const section = this.closest('[data-section-type]');
        const moreLink = this.querySelector('[data-swatches-more]');
      ...
      handleClicks(swatchLink) {
        ...
        swatchLink.addEventListener('click', (event) => {
          const isFocusEnabled = !document.body.classList.contains('no-outline');
          const variantId = swatchLink.getAttribute('data-swatch-variant');
      ...
      updateLinks(variantId) {
        this.linkElements = this.outer.querySelectorAll('[data-grid-link]');
        this.quickView = this.outer.querySelector('[data-button-quick-view]');
        ...
        if (this.quickView && theme.settings.quickBuy === 'quick_buy') {
          this.quickView.setAttribute('data-variant-id', variantId);
        }
      }

      replaceImages(id) {
        const imageSecondary = this.outer.querySelector(`[data-variant-secondary-id="${id}"]`);
        const gridImage = this.outer.querySelector(`[data-variant-id="${id}"]`);
        const gridImages = this.outer.querySelectorAll('[data-grid-image]');
        const currentGridImage = [...gridImages].find((image) => image.classList.contains('product-media__img--featured-visible'));
        ...
        if (gridImage) {
          ...
          const onAnimationEnd = () => {
            requestAnimationFrame(() => {
              currentGridImage.classList.remove('product-media__img--featured-visible');
              gridImage.classList.add('product-media__img--featured-visible');

              requestAnimationFrame(() => {
                imageSecondary.classList.remove('product-media__img--featured-visible');
              });
            });
          ...
          requestAnimationFrame(() => {
            imageSecondary.classList.add('product-media__img--featured-visible');
          });
